...
dependencies {
    ...
    if (isFabric) {
        ...
        compileOnly("com.terraformersmc:modmenu:15.0.2")
    } else {
        add("neoForge", "net.neoforged:neoforge:21.7.25-beta")
    }
...
tasks.register<Exec>("buildFabric") {
    group = "build"
    description = "Build Cerulean for Fabric 1.21.7"
    workingDir = rootDir
    if (isWindows) {
...
tasks.register<Exec>("buildNeoForge") {
    group = "build"
    description = "Build Cerulean for NeoForge 1.21.7"
    workingDir = rootDir
